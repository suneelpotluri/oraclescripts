...
select	max(L0.read_only) sort0,
	...
	(p.bytes/1048576)/abs(decode((p.completion_time - p.start_time),0,(1/86400),(p.completion_time - p.start_time)) * 86400) bkup_mbytes_sec,
	&&V_CLOSE_COMMENT
...
group by 
	...
	(p.bytes/1048576),
	(p.bytes/1048576)/abs(decode((p.completion_time - p.start_time),0,(1/86400),(p.completion_time - p.start_time)) * 86400) &&V_CLOSE_COMMENT
union all
