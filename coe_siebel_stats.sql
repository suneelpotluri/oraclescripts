...
CL TIMI;
REM
REM $Header: coe_siebel_stats.sql 11.4.4.6 2015/09/09 abel.macias carlos.sierra $
REM
REM Copyright (c) 1990-2015, Oracle. All rights reserved.
REM
...
REM
REM DESCRIPTION
REM   Collects schema object statistics for Siebel on 10g, 11g, 12c.
REM
REM PARAMETERS
...
DEF schema_owner = '&3';
PRO
PRO Please Wait...The package siebel_stats and supporting objects are being created.
PRO if session gets disconnected please review coe_siebel_stats.log and coe_gather_statistics.sql
...
CREATE OR REPLACE PACKAGE siebel_stats AS
  FUNCTION gather_siebel_stats (
    p_schema_owner   IN VARCHAR2 DEFAULT '&&schema_owner.',
...
CREATE OR REPLACE PACKAGE BODY siebel_stats AS
  ...
  IS
    ...
    l_exec VARCHAR2(12);
  BEGIN
    -- validates this is greater than 10g , else EXIT
    DECLARE
      dot1 NUMBER;
      dot2 NUMBER;
    BEGIN
    ...
    DECLARE
      cnt NUMBER;
    BEGIN
      l_schema_owner := UPPER(NVL(TRIM(' ' FROM p_schema_owner), '&&schema_owner.'));
    ...
    IF p_commands_only = 'N' THEN
      PIPE ROW('REM $Header: coe_gather_statistics.sql 11.4.4.6 2015/09/09 abel.macias carlos.sierra $');
    ...
    FOR i IN (SELECT t.owner, t.table_name, t.temporary, t.partitioned, t.num_rows, t.last_analyzed, s.stattype_locked, s.stale_stats
               ...
               WHERE t.owner = l_schema_owner
                 ...
                 AND t.table_name NOT LIKE 'DR$%' -- bug 8452962
                 AND t.table_name NOT LIKE 'DR#%' -- bug 8452962
                 ...
                 AND NOT EXISTS (
              SELECT NULL
                FROM sys.dba_external_tables e
    ...
    LOOP
      ...
      ELSE -- bigger tables (num_rows > rows_threshold)
        ...
        -- if 12g then use auto sample size instead (ok only if gathering with no histograms)
        -- this part has been delayed until 12c due to bug 10174050: AUTO_SAMPLE_SIZE is causing some histograms to miss important buckets
        IF l_rdbms_release >= 12 THEN
